...
public class ItemSourceComponentBuilder extends ItemComponentBuilder {
    ...
    public static class ItemSourceComponentBuilderPopup extends ItemComponentBuilderPopup {
        ...
        public Template makeTemplate() {
            return new SourceItemMachineComponent.Template(this.id.getValue(), this.mode.getValue(), this.capacity.intValue(), this.maxInput.intValue(), this.maxOutput.intValue(), this.baseTemplate().map(template -> template.filter).orElse(Filter.empty()), this.mode.getValue().getBaseConfig(), this.locked.selected());
        }
    }
}
